...
<div class="pull-right">
    ...
    <a href="javascript:end()" class="btn btn-primary"><i class="fa fa-check"></i>จบการขาย</a>
    ...
    window.open("{{URL::to('cart/complete')}}?cust_name="+$('#cust_name').val() + '&cust_email='+$('#cust_email').val(),"_blank")
     ...
     <script type="text/javascript">function end(){
    window.open("{{URL::to('cart/complete')}}?cust_name="+$('#cust_name').val() + '&cust_email='+$('#cust_email').val(),"_blank")
    window.location.href = "{{URL::to('cart/finish')}}"}</script>
</div>
</div>   
@endsection